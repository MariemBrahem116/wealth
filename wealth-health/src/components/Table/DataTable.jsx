...
/* css   */
import styles from '../Table/dataTable.module.css'
...
export default function DataTable({products,rowsPerPage}) {
  ...
  },[items,page,products.length,range.length,rowsPerPage]);
 ...
 */
  ...
  return (
        ...
        <tr>
          {employeeColumns.map((item,index) => (
            <th key={item.accessor+'-'+index}>
              <button type="button" onClick={() => requestSort(item.accessor)} className={`${styles[getClassNamesFor(item.accessor)]}`}>
                {item.Header}
              </button>
            </th>
        ))}
    ...
    { (slice.length === 0) &&(<p>No data available in table</p>) }
